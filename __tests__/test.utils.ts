...
const helloWorldCleanCode = fs.readFileSync(sourceFileWithoutComments).toString().trim();

describe('Util function tests', () => {
  describe('cleanCode', () => {
    it('Cleans Brainfuck code correctly', () => {
      const cleanedCode = cleanCode(helloWorldCode);
      expect(cleanedCode).toBe(helloWorldCleanCode);
    });
  });
  describe('genIndent', () => {
    it('Generates correct output', () => {
      expect(genIndent(0, 2, ' ')).toHaveLength(0);
      expect(genIndent(1, 2, ' ')).toHaveLength(2);
      expect(genIndent(2, 2, ' ')).toHaveLength(4);
    });
  });
});

...
function transpileToC(source) {
  ...
  const outputCodeArray = [
    ...
    'int main()',
    '{',
    `${genIndent(1, indentSize, indentChar)} char* cells = calloc(30000, sizeof(char));`,
  ...
  sourceArray.forEach((command) => {
    switch (command) {
      case '>':
        {
          const indent = genIndent(currentDepth, indentSize, indentChar);
          outputCodeArray.push(`${indent}position = (position + 1 < 30000) ? position + 1 : position;`);
        }
        break;
      ...
      case '<':
        {
          const indent = genIndent(currentDepth, indentSize, indentChar);
          outputCodeArray.push(`${indent}position = (position - 1 > 0) ? position - 1 : position;`);
        }
        break;
      ...
      case '+':
        {
          const indent = genIndent(currentDepth, indentSize, indentChar);
          outputCodeArray.push(`${indent}cells[position] = (cells[position] < 255) ? cells[position] + 1 : cells[position];`);
        }
        break;
      ...
      case '-':
        {
          const indent = genIndent(currentDepth, indentSize, indentChar);
          outputCodeArray.push(`${indent}cells[position] = (cells[position] > 0) ? cells[position] - 1 : cells[position];`);
        }
        break;

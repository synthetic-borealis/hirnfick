...
function transpileToPascal(source, programName = 'Hirnfick', indentSize = 1, indentChar = '\t') {
  ...
  sourceArray.forEach((command) => {
    ...
    switch (command) {
      ...
      case '<':
        outputCodeArray.push(`${indent}if position - 1 > ${cellsUBound} then dec(position);`);
        break;

      case '+':
        outputCodeArray.push(`${indent}if cells[position] + 1 < 255 then inc(cells[position]);`);
        break;

      case '-':
        outputCodeArray.push(`${indent}if cells[position] - 1 > 0 then dec(cells[position]);`);
        break;
